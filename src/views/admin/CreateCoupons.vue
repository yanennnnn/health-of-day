                    ...
                    <tbody>
                        <tr v-for="item in coupons" :key="item.id">
                            ...
                            <td >
                                ...
                                <button type="button" class="btn btn-outline-danger" @click="deleteModal(item)">刪除</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                            ...
                            <div class="col">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="num" class="form-control" id="percent"
                                        placeholder="請輸入折扣百分比"  v-model="tempCouponcode.percent" onkeyup="value=value.replace(/[^0-9]/g,'')">
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox"
                                        id="is_enabled" v-model="tempCouponcode.is_enabled" :true-value="1" :false-value="0">
        ...
        <div class="modal fade" id="delCouponcodeModal" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content border-0">
                    ...
                    <div class="modal-body">
                        是否刪除 <strong class="text-danger">{{ tempCouponcode.title }}</strong> 商品(刪除後將無法恢復)。
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-muted" data-dismiss="modal">取消</button>
...
export default {
    ...
    methods:{
        ...
        updateCouponcode(){
            ...
            if(!vm.isNew){
                api=`${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/admin/coupon/${vm.tempCouponcode.id}`;
                httpMethod='put';
                messageSuccess='編輯成功!';
                 messageDanger='編輯失敗!'
            }
            this.axios[httpMethod](api,{data:vm.tempCouponcode}).then((response) => {
        ...
        delCouponcode(){ //刪除優惠碼
            const vm = this;
            const api = `${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/admin/coupon/${vm.tempCouponcode.id}`;
