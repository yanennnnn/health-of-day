                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="item in products" :key="item.id">
                            <td scope="row">
                                <small class="rounded p-1" :class="textColor(item.category)">{{item.category}}</small>
                            </td>
                            <td>{{item.title}}</td>
                            <td>{{item.origin_price | currency}}</td>
                            <td :class="{'text-danger':item.origin_price!==item.price}">{{item.price | currency}}</td>
                            <td><span v-if="item.is_enabled" class="text-success">啟用</span>
                                <span v-else >未啟用</span>
                            </td>
                            <td>
                                <button type="button" class="btn btn-primary" @click.prevent="openModal(false,item)" >編輯</button>
                                <button type="button" class="btn btn-outline-danger" @click.prevent="deleteModal(item)">刪除</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                            ...
                            <div class="col-sm-4">
                                ...
                                <div class="form-group">
                                    <label for="customFile">或 上傳圖片
                                        ...
                                        <font-awesome-icon v-if="status.fileUploading" class="fa-spin" :icon="[ 'fas', 'spinner' ]" />
                                    </label>
...
export default {
    ...
    data(){
        return{
            ...
            tempProduct:{},
            status:{
                fileUploading:false//Modal內圖片上傳的Loading正常時不啟動
            },
            pagination: {},
            isLoading:false,
    ...
    methods: {
        ...
        textColor(item){
            ...
            switch (item) {
                case '經典商品':
                    return 'badge-atomic-tangerine';
                    break;
                 case '活力果汁':
                    return 'badge-middle-green-yellow';
                    break;
                 case '經典商品':
                    return 'badge-light';
                    break;
        ...
        openModal(isNew,item){
            ...
            $('#updateModal').modal('show');//手動打開動態視窗。 在動態視窗實際顯示之前返回給調用者（即在shown.bs.modal 事件發生之前）。
        },
        deleteModal(item){
